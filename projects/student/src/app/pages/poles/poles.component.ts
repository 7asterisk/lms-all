...
export class PolesComponent implements OnInit {
  ...
  newPole = { text: '', time: Date.now(), choices: [], studentId: '', courseId: '' };
  ...
  ngOnInit(): void {
    ...
    this.newPole.studentId = this.studentId;
  ...
  hasVoted(pole) {
    const i = pole.voted.findIndex(x => x.studentId === this.studentId);
    if (i > -1) {
      return true;
    }
    return false;
  ...
  selectChoice(poleId, chId) {
    ...
    this.allPole[poleIndex].voted.push({ studentId: this.studentId });
